# Otopair — Engineering Update

**Date:** 2026-06-03 · **Branch:** `waleed-flagship`
**Scope:** (1) Flagship home page + Oto agent (full PR description embedded below), (2) Enrichment pipeline fixes — parts pricing + service/labor times (this session), (3) Up next: aligning per-service parts pulling with the **Otopair Service Parts Reference**.

---

# Part 1 — Flagship Home Page PR

> Full PR description (`PR_FLAGSHIP_HOME_PAGE.md`) embedded verbatim below.
...
# Part 2 — Enrichment Pipeline: Parts Pricing + Service Times (this session)

Two MVP blockers, both rebuilt on the same principle: **replace "one winning value chosen by source confidence" with "append-only per-source observations → robust median, internal/empirical data preferred."**

## 2a. Parts pricing — the "$11 rotor" bug, root-caused and fixed

### The bug
The pipeline stored the **"You Save $X" discount** (e.g. $11) or mis-associated values instead of the real part price (~$100+). **Root cause:** Firecrawl returned *markdown only* — the real price, struck-through MSRP, and "You Save" figure collapsed into ambiguous text before an LLM read them, and the LLM grabbed the wrong number. A second latent bug: the structured-registry scrape path only ran for **BMW** (`scraper.ts` dispatch) — every other make fell through to open-web markdown.

### The fix — deterministic, domain-agnostic price extraction
- **Fetch raw HTML** (Firecrawl `formats: ["markdown","rawHtml"]`) on both the registry path and open-web search results.
- **New `priceParser.ts`** — layered, deterministic, no LLM:
  1. **JSON-LD** `schema.org/Product → offers.price` (the universal e-commerce standard) — handles `@graph`, offer arrays (takes the **lowest**, never first/MSRP), `lowPrice`, `priceSpecification`, price-0 fallthrough.
  2. Microdata / OpenGraph meta tags.
  3. Per-domain DOM selector registry — fires **only** for registered domains (RevolutionParts family: reads `.price-section-price`, explicitly never `.price-section-retail`/`-save`).
- **Gather from anywhere — not hardcoded to RevolutionParts.** Any domain exposing structured data becomes its own `part_prices` source (validated live on `rmeuropean.com`, a non-RevolutionParts site). Strict association: prices attach only on an exact normalized OEM-number match — ambiguous/cross-sell products are skipped, never guessed.
- **The LLM no longer prices parts.** Batch-1 pricing extraction removed from the prompt; the Batch-2 path survives only as a tagged `llm_estimate` fallback and is skipped for any part the parser already priced.
- Parsed prices ride `scrape_cache` (`part_prices_json` + `format_version=2` — stale markdown-only cache rows automatically re-fetch).

### Validated against the real world (full findings: `PARTS_PRICING_VALIDATION.md`)

| Part | Stored (wrong) | Parser extracts | Live sale price |
|---|---|---|---|
| Nissan Rogue cabin filter | ~$15 | **$31.14** | $31.14 ✅ |
| Nissan Rogue spark plug | $70.89 | **$24.78** | $24.78 (You Save = $11.10 — the exact trap, avoided) ✅ |
| BMW M550i front rotor | $358 (scraped off a *pads* page) | **$251.83** | $251.83 ✅ |
| BMW cabin filter (`rmeuropean.com`) | $15.92 | **$32.04** | $32.04 ✅ (non-RevolutionParts source) |

**To the cent, on every page tested**, cross-checked against independent retailers. One rotor page alone yielded 20 priced SKUs for the cross-source median.

### Corpus repair + cutover (flag-gated, reversible)
- **`backfills.ts`** — paginated, self-continuing backfill: writes correct rows **first**, then prunes legacy poison rows (snapshotted to `price_backfill_log` before delete — fully reversible; a part can never be left priceless). Gated by `BACKFILL_PARTS_PRICES`, dry-run by default.
- **Quote-time median** — all consumers (`serviceParts`, `booking_quotes`, `job_actuals`) route through one `quoteUnitPrice`; `PARTS_PRICE_SOURCE=median` flips quotes to the robust cross-source median (default `average` = inert until the backfill is signed off). Mechanics' ± price nudges stay a separate stream (`part_snapshots`) — never double-counted.

## 2b. Service/labor times — VDB de-throned

### The bug
Labor times were inconsistent vehicle-to-vehicle because Vehicle Databases sat at 0.90 confidence and "highest confidence wins" — making an unreliable provider the de-facto source of truth, with its summed sub-action mapping amplifying noise.

### The fix — observations + median + empirical override
- **New `labor_observations` table** (append-only): VDB writes are now **one low-weight vote** (`vdb_repair_estimates`), alongside LLM web/training baselines, at engine-family granularity.
- **`recomputeLaborForConfigService`** (`lib/labor_aggregation.ts`): `book_hours` = unweighted robust median across catalog observations, clamped 0.1–8.0h, rounded to 0.1. A lone VDB outlier can no longer dominate.
- **Empirical override, gaming-resistant:** real post-job actuals override book time only at **N ≥ 3** (`LABOR_EMPIRICAL_MIN_SAMPLES`), restricted to **single-service bookings** (a multi-service booking total can't be honestly split). The resolver already prefers `empirical_hours` — our own data automatically takes over as jobs accrue.
- **Cron** (`recompute-recent-labor`, every 6h): configs touched by recent quotes/actuals recompute continuously — internal data improves without re-enrichment.
- Shared **`lib/robustStats.ts`** (median, MAD outlier rejection) now powers both parts and labor.

## 2c. How we know it's correct

- **22/22 unit tests** (parser + robust stats), `tsc` clean.
- **Live probes** against real pages via the actual pipeline fetch path (temporary `pricePilot.ts`), cross-checked manually on the web.
- **Independent 4-agent adversarial audit** tried to break the diff. It confirmed the core claims and found **3 HIGH issues — all fixed**: the backfill could have deleted *good* `llm_estimate` rows (now type-guarded), offer arrays returned the first price instead of the lowest, and a misleading "weighted" comment on labor. Plus 6 medium/low fixes (price-0 fallthrough, `priceSpecification`, per-domain dedup breadth, `book_only` recompute path, etc.). Re-probed after fixes: unchanged, still correct.
- **Honest deferred list** documented (brand-prefixed SKUs, `sale`-row preference at read time, `job_actuals` index) — nothing silently dropped.

## 2d. Rollout state

| Step | Status |
|---|---|
| Code (parser, observations, backfill, flags) | ✅ Done on `waleed-flagship`, tests green |
| Real-world validation | ✅ To-the-cent on every probe |
| Deploy to `temurbek` / production | ⏳ Pending |
| Run backfill (`BACKFILL_PARTS_PRICES=on`, dry-run → live) | ⏳ Pending deploy |
| Flip `PARTS_PRICE_SOURCE=median` | ⏳ After backfill + shadow-diff sign-off |
| Delete temporary `pricePilot.ts` probe | ⏳ Awaiting sign-off |

**Key property:** everything is additive + flag-gated. New enrichments write correct prices the moment the code deploys; the existing corpus self-heals on re-enrichment for same-domain rows and is fully cleaned by the (reversible) backfill; the quote-time behavior change is a single env-var toggle, instantly revertible.

---

# Part 3 — Coming Up: pull parts per the **Otopair Service Parts Reference**

**Source of truth:** `Otopair_Service_Parts_Reference (1).pdf` (internal handoff, May 2026) — the standard parts breakdown for **all 23 canonical services**: exactly what goes on the invoice for each job. It's vehicle-agnostic (the parts a service consumes are consistent across cars; only brand and spec change), and it's been verified by two research sweeps as the industry-standard breakdown.

### What the reference defines

- **8 labor-only services** — no parts billed at all: Diagnostic Scan, Pre-Purchase Inspection (excluded from MVP), CEL Diagnosis, NY State Inspection, NY Emissions Test, Tire Rotation, Wheel Alignment, Battery Test.
- **15 parts-bearing services**, each with a tiered parts list:
  - **Core** — on essentially every invoice (e.g. oil change = oil + filter; rotor replacement = 2 rotors **+ pads + grease**, per axle).
  - **As-needed / conditional** — the discovery items that make a service quote a *range* instead of a lock (crush washers, ignition coils, wear sensors on Euro cars, hardware kits, thermostat-if-bad, TPMS valve kits, …).
  - **Kits / variants** — timing belt bundles (water pump + tensioner + idlers + seals), transmission drain-and-fill vs. full pan service, fuel-system light/medium/deep tiers.

### The task

Edit how the pipeline **pulls parts information per service** so the gathered set matches this reference, instead of whatever ad-hoc set enrichment currently extracts per service:

1. **Coverage** — for each parts-bearing service, pull (and price) every **core** item, not just the headline part: e.g. brake jobs must include pads + grease + hardware/wear-sensor where applicable; oil changes include the crush washer / housing O-ring where equipped.
2. **Tier tagging** — carry the reference's core / as-needed / kit distinction into `service_parts`, so quotes can show a locked core price + a discovery range (this is the data model behind "a service ranges instead of locks").
3. **Labor-only enforcement** — the 8 labor-only services must never accumulate part rows (and existing stray rows should be cleaned).
4. **Conditional rules** — encode the vehicle-conditional logic the reference spells out (cartridge vs. spin-on filters, belt- vs. chain-driven timing, CVT vs. AT, Euro wear sensors, TPMS-equipped ≈ all US 2008+, electric PS = N/A).

This slots directly on top of Part 2: the deterministic price parser + cross-source median price **whatever parts we ask for** — this task fixes **which parts we ask for**, completing the invoice-accuracy story end-to-end.

---

# One-slide summary

> **Home page:** a live conversational AI demo that decodes your car, walks you through booking, and captures pre-signup leads that auto-claim at signup — with a validated, injection-safe dynamic card system.
>
> **Pipeline:** parts prices and labor times no longer trust any single provider. Prices are extracted deterministically from structured page data (validated to the cent against the real world), labor is a robust median over observations with our own post-job data taking over as it accrues — all flag-gated, reversible, and adversarially audited.
>
> **Next:** make every service pull its full, correctly-tiered parts list per the Service Parts Reference — so the right parts get the right prices on every invoice.
