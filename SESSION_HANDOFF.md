...
0. **Retire/rewrite `diagnoseVin.ts`'s 4 `online_discount` writers** (lines 487/674/983/1296) — dead from the pipeline (manual CLI only, verified) but a foot-gun: a director could re-mint poison. Route them through `reextractPartPrice` or delete. Low priority.
