/**
 * repairpalEndpointProbe.ts — one-shot GATE check: can a Convex cloud action
 * reach the RepairPal next-api endpoints? It works from our dev IP, but
 * Cloudflare may treat Convex datacenter IPs differently. Run this BEFORE
 * trusting the live-in-pipeline resolver:
 *   npx convex run vehicleEnrichment/repairpalEndpointProbe:probe
...
export const probe = internalAction({
  ...
  handler: async (): Promise<Record<string, unknown>> => {
    ...
    try {
      const r = await fetch("https://repairpal.com/next-api/estimator-flow/makes?year=2021", { headers });
    ...
    try {
      ...
      const r = await fetch(
        "https://repairpal.com/next-api/estimator-flow/estimate?baseVehicleId=78290&serviceId=107&zipCode=10001&scheduled=0",
        { headers },
      );
