...
describe("mergeLaborSources", () => {
  it("emits one weighted observation per (service, source) and skips empties", () => {
    const rows = mergeLaborSources({
      ...
      repairpalEndpoint:{ oil_change: 0.5 },
    });
    // oil_change: olp 0.5 (w0.7), web 0.6 (w0.6), repairpal 0.55 (w0.4), endpoint 0.5 (w0.9); spark_plugs: olp only
    expect(rows).toEqual(expect.arrayContaining([
      ...
      { service: "oil_change", source: "repairpal_endpoint", hours: 0.5, weight: 0.9 },
