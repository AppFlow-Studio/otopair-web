...
const REDUCTO_BASE = "https://api.reducto.ai/v1";
...
async function callReductoExtract(
  ...
  try {
    ...
    if (body.error) throw new Error(`Reducto error: ${body.error}`);

    return {
      payload: (body.result ?? {}) as Record<string, unknown>,
      confidenceMap: body.confidence_scores ?? {},
      jobId: body.job_id,
    };
  } finally {
    clearTimeout(timeout);
