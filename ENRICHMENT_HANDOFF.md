...
**RepairPal URL findings (Jun 10, probed live):** year-path pages are real and generation-scoped (jetta yearless 0.42h vs jetta/2020 0.58h — the year-first candidates are earning their keep); Waleed's `?baseVehicleId=&serviceId=` query form is the most precise (exact variant) — future upgrade: scrape the page's vehicle-picker once per nameplate to cache `(year, model) → baseVehicleId`, then use the query form everywhere. The rotor pages 500 in Firecrawl in EVERY URL shape (yearless/year/curl-200-but-JS-rendered) — try Firecrawl `waitFor`/options or the baseVehicleId form with a rotor serviceId; tier_estimate covers rotors meanwhile.
