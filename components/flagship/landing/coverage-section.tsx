...
/* Stats-card type. Base classes are the ≥sm sidebar exactly as before; the
   `max-tab:` additions are the phone frame's 28/26/26 line grid (label 13/28,
   value 40 Romie-Medium → Petrona 500 on a 26 line, unit 17/26, sub 15/26). */
const STAT_LABEL = "text-[13px] tracking-[0.05em] text-[#777169] max-tab:leading-[28px]";
const STAT_LINE = "mt-2 flex items-baseline gap-1.5 max-tab:mt-[15px] max-tab:h-[26px]";
const STAT_VALUE = "text-[40px] leading-none text-[#1a1a1a] max-tab:leading-[26px] max-tab:font-medium!";
const STAT_UNIT = "text-[17px] text-[#777169] max-tab:leading-[26px]";
const STAT_SUB = "mt-1 text-[15px] leading-[26px] text-[#777169] max-tab:mt-0";
...
const STAT_BLOCK = "mt-7 px-6 pt-7 max-tab:mt-[26px] max-tab:border-t-0! max-tab:px-[44px] max-tab:pt-0";
const STAT_RULE = "mx-[44px] mt-[32px] h-[1.5px] w-[260px] max-w-[calc(100%-88px)] bg-white tab:hidden";
...
export default function CoverageSection() {
  ...
  return (
    <section
      id="coverage"
      className="mx-auto w-full max-w-[1440px] px-4 pt-20 max-tab:px-[27px] max-tab:pt-[60px] tab:px-10 tab:pt-28 lg:px-[78px]"
    >
      {/* Mobile (below sm) follows the Figma phone frame (iPhone 16 & 17 Pro - 7,
          ...
          stats card at a 28px inset. Every mobile value rides a `max-tab:`
      ...
      <div className="mx-auto w-full max-w-[1190px]">
        <Reveal>
          <h2
            className="max-w-[760px] text-[40px] leading-[1.02] text-[#1a1a1a] max-tab:text-[28px] max-tab:leading-[28px] max-tab:font-normal! max-tab:[font-size-adjust:none]! tab:text-[54px] lg:text-[68px] lg:leading-[68px]"
            style={serifDisplay}
          >
        ...
        <Reveal delay={0.1} className="max-tab:hidden">
          <div className="mt-10 tab:mt-12 lg:mt-[48px]">
            ...
            <div className="grid grid-cols-2 gap-2 tab:grid-cols-3 lg:hidden">
              {BOROUGHS.map((b) => (
        ...
        <Reveal delay={0.15}>
          <div className="mt-8 grid grid-cols-1 gap-6 max-tab:mt-[25px] max-tab:gap-[44px] lg:mt-[40px] lg:grid-cols-[minmax(0,1fr)_250px] lg:gap-[41px]">
            {/* Frame: map at x=22 (5px outside the 27px text inset), 342 tall. */}
            <div
              ref={mapCellRef}
              className="relative h-[380px] overflow-hidden rounded-[8px] max-tab:-mx-[5px] max-tab:h-[342px] tab:h-[520px] lg:h-auto lg:min-h-[720px]"
            >
            ...
            <div className="flex flex-col overflow-hidden rounded-[8px] bg-[#EBF5FB] max-tab:mx-px max-tab:rounded-[20px]">
              <div className="px-6 pt-7 max-tab:px-[44px] max-tab:pt-[32px]">
                <p className={STAT_LABEL}>VERIFIED SHOPS</p>
                <p className={`${STAT_LINE} max-tab:gap-[9px]`}>
              ...
              <div className={STAT_BLOCK} style={{ borderTop: `1px solid ${HAIRLINE}` }}>
                ...
                <p className={`${STAT_LINE} max-tab:gap-[5px]`}>
                  <span className={STAT_VALUE} style={serif}>
              ...
              <div className="mt-8 flex items-center gap-2 bg-[#4B82A5] px-6 py-3 max-tab:h-[45px] max-tab:px-[45px] max-tab:py-0">
                <span className="h-1.5 w-1.5 rounded-full bg-white/85 max-tab:hidden" />
                <span className="text-[13px] tracking-[0.08em] text-white max-tab:text-[15px] max-tab:leading-[28px] max-tab:tracking-[0.05em]">
                  LIVE SIGNUPS
                </span>
              </div>
              ...
              <div className="flex-1 px-6 pb-8 pt-6 max-tab:px-[44px] max-tab:pb-[41px] max-tab:pt-[29px]">
                {signups.map((s, i) => (
                  <div key={i} className={i > 0 ? "mt-6 max-tab:mt-[21px]" : ""}>
                    <p className="text-[15px] tracking-[0.05em] text-[#1a1a1a] max-tab:leading-[28px]">{s.name}</p>
                    <p className="text-[12px] tracking-[0.05em] text-[#777169]/60 max-tab:-mt-[9px] max-tab:leading-[28px] max-tab:text-[#777169]/50">
                      {s.when}
                    </p>
                    <span
                      className={`mt-2 inline-block w-full rounded-[10px] px-3.5 py-0.5 text-[12px] font-medium tracking-[0.05em] max-tab:mt-[2px] max-tab:block max-tab:h-[20px] max-tab:w-[227px] max-tab:max-w-full max-tab:px-[19px] max-tab:py-0 max-tab:leading-[20px] ${
                        s.badge === "Verified"
