...
9. ✅ **FIXED** (`fix(parts): diagnoseVin writers route through the Batch-2 verification contract`, Jun 10) — ~~`diagnoseVin`'s 4 `online_discount` writers can OVERWRITE corrected `sale` rows back to poison~~. All 4 price writers now go through a shared `writeVerifiedLlmPrice` helper applying the Batch-2 two-tier contract (`reextractPartPrice` + `isAffirmativeRejection`): page-verified → `sale`, page-refuted → `unverified`, passive failure → fail-open `llm_estimate`. `online_discount` is no longer written anywhere outside comments.
...
17. 🟡 **PARTLY FIXED** (Jun 10) — ~~case-sensitive `fuel_type === "electric"` never fires (NHTSA writes "Electric")~~ — both the ICE-only and hydraulic-PS gates in `services/applicability.ts` now compare case-insensitively (exact match kept deliberately: PHEV/HEV strings contain "Electric" but still have an ICE). Tests: `tests/serviceApplicability.test.ts`. **Still open:** enrichment-side EV field-nulling rules (`applicabilityRules.ts` has NO fuel-type rule at all — matters once the F-150 Lightning configs enrich).

18. ✅ **FIXED** (Jun 10, same commit as 17) — ~~Oto's `list_services_for_vehicle` returns the whole 23-service catalog unfiltered~~ — new IDOR-safe `oto/applicableServices.listServicesForUserVehicle` filters through the SAME `isServiceApplicable` rules as the booking surface; fails OPEN to the full catalog when the vehicle/config can't be resolved (tool contract stays total). Tests: `tests/otoApplicableServices.test.ts`.
...
- ✅ FIXED (Jun 10) — ~~Every "priced parts" coverage metric counts poison rows~~ — `getPricedPartCount` now requires ≥1 trusted (non-poison) row per part via the shared `lib/priceTypes` vocabulary. Tests: `tests/pricedPartCount.test.ts`. (The `v3queries.ts:352-359` cite in the original finding was this query.)
- ✅ FIXED (Jun 10) — NEW finding, found live during the Atlas relabor: `ctx.runQuery` serializes an `undefined` query return to `null`, and `resolveLaborSibling`'s `v.optional(v.string())` rejects null — any car WITHOUT a chassis code threw in sibling resolution (latent stuck-config route in `_pollBatch2V3`'s labor block). Both call sites normalize `?? undefined`.
...
- ✅ FIXED (Jun 10) — ~~Quote engine refuses differential service for RWD/4WD~~ — exported `hasServiceableDifferential` (AWD/4WD/4x4/RWD true; FWD false; unknown false fail-safe). Tests: `tests/differentialApplicability.test.ts`.
