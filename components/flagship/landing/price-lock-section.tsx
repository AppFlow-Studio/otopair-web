...
/* ------------------------------------------------------------------ */
/* The three roles — hovering one swaps the graphic on the left.       */
/* ------------------------------------------------------------------ */
const ROLES = [
  {
    num: "01.",
...
export default function PriceLockSection() {
  ...
  // DriversPanel (24400), ShopsPanel (27100) and OtoPanel (OTO_LOOP_MS, 37700).
  const STORY_MS = [24100, 26800, 37400];
  ...
  return (
    ...
    >
      <Reveal>
        <h2
          className="max-w-[920px] text-[40px] leading-[1.02] text-[#1a1a1a] sm:text-[54px] lg:text-[68px]"
          style={serif}
        >
      ...
      <div className="mt-16 grid grid-cols-1 gap-12 sm:mt-24 lg:grid-cols-[minmax(0,0.86fr)_minmax(0,1fr)] lg:items-center lg:gap-16">
        {/* The story stage */}
        ...
        >
          {/* Sliding white highlight behind the active row (lg only) */}
          <motion.div
            aria-hidden
            className="pointer-events-none absolute inset-x-0 top-0 hidden rounded-[16px] bg-white/70 shadow-[0_2px_24px_rgba(26,26,26,0.06)] lg:block"
            style={{ height: "33.333%" }}
            initial={false}
          ...
          {ROLES.map((role, i) => {
            ...
            return (
              <div
                ...
                className="relative flex cursor-pointer flex-col justify-center rounded-[16px] px-0 py-8 outline-none focus-visible:ring-1 focus-visible:ring-[#1a1a1a]/30 lg:h-[33.333%] lg:min-h-[168px] lg:px-8"
              >
                <p
