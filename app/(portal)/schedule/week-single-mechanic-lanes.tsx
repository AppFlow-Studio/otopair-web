...
interface WeekSingleMechanicLanesProps {
  mechanic: Mechanic;
...
export default function WeekSingleMechanicLanes({
  mechanic,
  events,
  weekStart,
  minTime,
  ...
  const { bookingsByDay, blockedByDay } = useMemo(() => {
    ...
    for (const day of days) {
      bkMap.set(day.dateStr, []);
      blMap.set(day.dateStr, []);
    }
    for (const ev of events) {
      const dateStr = dateToString(ev.start);
      if (ev.type === "blocked") {
        blMap.get(dateStr)?.push(ev);
      } else {
        bkMap.get(dateStr)?.push(ev);
      }
    }
    return { bookingsByDay: bkMap, blockedByDay: blMap };
  }, [days, events]);
  ...
  const dropTargetRef = useRef<{ dayIndex: number; slotIndex: number } | null>(null);
  ...
  const getDropTarget = useCallback((clientX: number, clientY: number) => {
    ...
    const colWidth = (container.scrollWidth - GUTTER_WIDTH) / ds.length;
    const dayIndex = Math.min(Math.max(Math.floor(gridX / colWidth), 0), ds.length - 1);
  ...
  const handlePointerDown = useCallback((e: React.PointerEvent, ev: CalendarEvent) => {
    ...
    function createFloating() {
      ...
      Object.assign(nameP.style, { fontWeight: "500", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" });
    ...
    function onMove(me: PointerEvent) {
      ...
      setDropTarget((prev) => {
        ...
        if (target && prev && target.dayIndex === prev.dayIndex && target.slotIndex === prev.slotIndex) return prev;
        return target;
      });
    }
    ...
    function onUp() {
      ...
      if (target) {
        ...
        if (slot && day) {
          ...
          // Forbidden: another booking in target day at that time
          const otherBookings = eventsRef.current.filter(
            (be) => be.type === "booking" && be.id !== ev.id && dateToString(be.start) === day.dateStr,
          );
          ...
          if (onProposeRescheduleRef.current) {
            const newTime = dropTime;
            const originalTime = formatHHMM(ev.start.getHours(), ev.start.getMinutes());
            const newDate = day.dateStr;
            const originalDate = dateToString(ev.start);
            const timeChanged = newTime !== originalTime;
            const dateChanged = newDate !== originalDate;
            const mech = mechanicRef.current;
            ...
            if (timeChanged || dateChanged) {
              onProposeRescheduleRef.current({
                ...
                originalDate,
                originalTime,
                originalMechanicId: mech._id,
                originalMechanicName: mech.name,
                newDate,
                newTime,
                newMechanicId: mech._id,
  ...
  return (
    ...
    >
      <div
        className="flex"
        style={{ minWidth: GUTTER_WIDTH + 7 * 140 }}
      >
        {/* Time gutter */}
            ...
            {slots.map((s, i) => {
              ...
              return (
                <div
                  ...
                  style={{ top: i * ROW_HEIGHT + 2, width: GUTTER_WIDTH, opacity: label.isHalf ? 0.45 : 1 }}
                >
        ...
        {/* Day columns */}
        {days.map((day, dayIdx) => {
          const dayBookings = bookingsByDay.get(day.dateStr) ?? [];
          const dayBlocked = blockedByDay.get(day.dateStr) ?? [];

          return (
            <div
              key={day.dateStr}
              className={`flex-1 min-w-[140px] ${dayIdx < days.length - 1 ? "border-r border-border" : ""}`}
            >
              {/* Column header */}
              <div
                className="flex flex-col items-center justify-center border-b border-border bg-card sticky top-0 z-30 px-2 py-2 gap-0.5"
                style={{ height: HEADER_HEIGHT }}
              >
                <div className="flex h-12 flex-col items-center justify-between">
                  <span className={`text-[11px] font-medium uppercase tracking-wider leading-3 ${day.isToday ? "text-primary" : "text-muted-foreground"}`}>
                    {day.dayName}
                  </span>
                  <span
                  ...
                  <span
                    className={`h-3 text-[10px] leading-3 text-muted-foreground ${
                      dayBookings.length > 0 ? "" : "invisible"
                    }`}
                  >
              ...
              >
                ...
                {slots.map((s, i) => (
                  <div
                    ...
                    className={`absolute left-0 right-0 pointer-events-none ${
                      s.minute === 0 ? "border-t border-border" : "border-t border-border/40"
                    }`}
                    ...
                    <div
                      ...
                      onClick={() => {
                        ...
                        onSelectBlocked({
                          slotId: bl.slotId,
                          date: day.dateStr,
                          startTime: formatHHMM(bl.start.getHours(), bl.start.getMinutes()),
                          endTime: formatHHMM(bl.end.getHours(), bl.end.getMinutes()),
                ...
                {/* Current time indicator — spans all columns so it forms one continuous line */}
                {showNowLine && (
                  <div
                ...
                {/* Event blocks */}
                {dayBookings.map((ev) => {
                  const evStartMin = minutesFromBase(startHour, startMinute, ev.start.getHours(), ev.start.getMinutes());
